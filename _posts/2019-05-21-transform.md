...
- 使用齐次表示法，方向向量表示为v = (vx,vy,vz,0),点表示为**v** = (vx,vy,vz,1)。
...
- 如下图，给出了一个平移变换例子。注意，方向向量**v**= (vx,vy,vz,0)不受乘以**T**的影响，因为方向向量不能被平移。相反，点和向量都受到其余仿射变换的影响。平移矩阵的逆矩阵$T^1(t)=T(-t)$ = **T**(-t)，也就是说向量**t**是负的。
